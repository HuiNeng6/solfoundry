...
from app.models.submission import (
    SubmissionDB, SubmissionStatus, MatchConfidence,
    SubmissionCreate, SubmissionUpdate, SubmissionListResponse,
    SubmissionListItem, SubmissionSearchParams, SubmissionStats, MatchResult,
)
...
class SubmissionService:
    ...
    async def create_submission(self, contributor_id: str, data: SubmissionCreate) -> Tuple[SubmissionDB, Optional[MatchResult]]:
        pr_info = self._parse_pr_url(data.pr_url)
        submission = SubmissionDB(
            contributor_id=contributor_id, contributor_wallet=data.contributor_wallet,
            pr_url=data.pr_url, pr_number=pr_info.get('pr_number'), pr_repo=pr_info.get('repo'),
            description=data.description, evidence=data.evidence, status=SubmissionStatus.PENDING.value,
        )
        ...
        if submission.bounty_id:
            ...
            if bounty:
                match_result = MatchResult(
                    bounty_id=str(submission.bounty_id), bounty_title=bounty.title,
                    match_score=submission.match_score or 0.0,
                    confidence=submission.match_confidence or MatchConfidence.LOW.value,
                    reasons=submission.match_reasons, github_issue_url=bounty.github_issue_url,
                )
        return submission, match_result
    
    async def update_submission(self, submission_id: str, data: SubmissionUpdate, reviewer_id: Optional[str] = None) -> Optional[SubmissionDB]:
    ...
    async def list_submissions(self, params: SubmissionSearchParams) -> SubmissionListResponse:
    ...
    async def get_contributor_stats(self, contributor_id: str) -> SubmissionStats:
        total_query = select(func.count(SubmissionDB.id)).where(SubmissionDB.contributor_id == contributor_id)
        total_result = await self.db.execute(total_query)
        total = total_result.scalar() or 0
        status_counts = {}
        for status in SubmissionStatus:
            count_query = select(func.count(SubmissionDB.id)).where(
                and_(SubmissionDB.contributor_id == contributor_id, SubmissionDB.status == status.value)
            )
        ...
        earnings_query = select(func.sum(SubmissionDB.reward_amount)).where(
            and_(SubmissionDB.contributor_id == contributor_id, SubmissionDB.status == SubmissionStatus.PAID.value)
        )
        ...
        if approved_or_rejected > 0:
            approved = status_counts.get(SubmissionStatus.APPROVED.value, 0) + status_counts.get(SubmissionStatus.PAID.value, 0)
            approval_rate = approved / approved_or_rejected
        return SubmissionStats(
            total_submissions=total,
            pending=status_counts.get(SubmissionStatus.PENDING.value, 0) + status_counts.get(SubmissionStatus.MATCHED.value, 0) + status_counts.get(SubmissionStatus.REVIEWING.value, 0),
            ...
            total_earnings=total_earnings, approval_rate=round(approval_rate, 3),
        )
    
    async def get_bounty_submissions(self, bounty_id: str) -> List[SubmissionDB]:
        query = select(SubmissionDB).where(SubmissionDB.bounty_id == bounty_id).order_by(SubmissionDB.created_at.desc())
        result = await self.db.execute(query)
    ...
    def _parse_pr_url(self, url: str) -> dict:
        ...
        if match:
            ...
            return {'owner': owner, 'repo': f"{owner}/{repo}", 'pr_number': int(pr_number)}
        return {}
    ...
    async def _auto_match_bounty(self, submission: SubmissionDB) -> Optional[MatchResult]:
        if not submission.pr_repo or not submission.pr_number:
            return None
        query = select(BountyDB).where(BountyDB.status.in_([BountyStatus.OPEN.value, BountyStatus.CLAIMED.value]))
        result = await self.db.execute(query)
        ...
        for bounty in bounties:
            ...
            if score > best_score:
                ...
                best_match = MatchResult(
                    bounty_id=str(bounty.id), bounty_title=bounty.title,
                    match_score=score, confidence=self._get_confidence(score),
                    reasons=reasons, github_issue_url=bounty.github_issue_url,
                )
        if best_match and best_score >= 0.5:
            return best_match
        return None
    
    def _calculate_match_score(self, submission: SubmissionDB, bounty: BountyDB) -> Tuple[float, List[str]]:
        score = 0.0
        reasons = []
        ...
        if bounty.github_issue_url:
            ...
            if issue_match:
                reasons.append(f"Bounty linked to issue #{int(issue_match.group(1))}")
    ...
    def _get_sort_column(self, sort: str):
        return {'newest': SubmissionDB.created_at, 'oldest': SubmissionDB.created_at,
                'status': SubmissionDB.status, 'reward': SubmissionDB.reward_amount}.get(sort, SubmissionDB.created_at)
