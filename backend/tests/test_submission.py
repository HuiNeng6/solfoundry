...
class TestSubmissionCreation:
    ...
    async def test_create_submission_with_pr_url(self, client, contributor_id, test_wallet):
        response = await client.post(
            ...
            json={"pr_url": "https://github.com/test-owner/test-repo/pull/123", "contributor_wallet": test_wallet}
        )
    ...
    async def test_create_submission_with_bounty_id(self, client, contributor_id, test_wallet, test_bounty):
        response = await client.post(
            "/api/submissions/",
            params={"contributor_id": contributor_id},
            json={"pr_url": "https://github.com/test-owner/test-repo/pull/124", "contributor_wallet": test_wallet, "bounty_id": str(test_bounty.id)}
        )
        assert response.status_code == 201
        data = response.json()
    ...
    async def test_create_submission_invalid_url(self, client, contributor_id, test_wallet):
        response = await client.post(
            "/api/submissions/",
            params={"contributor_id": contributor_id},
            json={"pr_url": "https://not-github.com/something", "contributor_wallet": test_wallet}
        )
...
class TestSubmissionRetrieval:
    ...
    async def test_get_submission(self, client, contributor_id, test_wallet):
        create_response = await client.post(
            "/api/submissions/",
            params={"contributor_id": contributor_id},
            json={"pr_url": "https://github.com/owner/repo/pull/1", "contributor_wallet": test_wallet}
        )
    ...
    async def test_list_submissions(self, client, contributor_id, test_wallet):
        for i in range(3):
            await client.post(
                "/api/submissions/",
                params={"contributor_id": contributor_id},
                json={"pr_url": f"https://github.com/owner/repo/pull/{i}", "contributor_wallet": test_wallet}
            )
...
class TestAutoMatching:
    ...
    async def test_auto_match_by_repo(self, client, contributor_id, test_wallet, test_bounty):
        response = await client.post(
            "/api/submissions/",
            params={"contributor_id": contributor_id},
            json={"pr_url": f"https://github.com/{test_bounty.github_repo}/pull/999", "contributor_wallet": test_wallet}
        )
        assert response.status_code == 201
        data = response.json()
