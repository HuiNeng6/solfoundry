...
async def test_bounty_event_template(service):
    ...
    await svc._process_send(
        "dev@sol.com", "Bounty Closed", "bounty_event", ctx, "bounty_event"
    )
...
async def test_unsubscribe_and_preferences(service):
    ...
    res1 = await svc._process_send(
        "picky@user.com", "Update", "welcome", {}, "bounty_event"
    )
...
async def test_rate_limiting(service):
    ...
    for i in range(10):
        res = await svc._process_send(
            "spammer@sol.com", f"Spam {i}", "welcome", {}, "general"
        )
        assert res is True

    # The 11th should be blocked
    res = await svc._process_send(
        "spammer@sol.com", "Spam 11", "welcome", {}, "general"
    )
    assert res is False
...
async def test_async_queue_enqueuing():
    ...
    res = await svc.send_email_async(
        "queue@sol.com", "Subject", "welcome", {}, "general"
    )
    assert res is True
